...
def slimulation(repnumber):
    ...
    if not exists("%s_%s_overlaid_ms.out" % (options.prefix, str(repnumber))):
        ...
        if options.paramsfile != 'None':
            ...
            currentparams = parameters[linenumber].strip()
        ...
        # recapitate
        ts = pyslim.recapitate(orig_ts, recombination_rate=5e-6, ancestral_Ne=1250)
