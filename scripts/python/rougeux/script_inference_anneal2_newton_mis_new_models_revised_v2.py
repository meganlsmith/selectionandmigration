...
def callmodel(func, data, output_file, modeldemo, ll_opt_dic, nbparam_dic,
          ...
          pts_l=None, ns=None,outputname=None, verbose=False, maxiter=400, 
    ...
    if optimizationstate == "anneal_hot" :
        ...
        popt, ll_model = dadi.Inference.opt(p0, data, func_ex, pts_l,
                                        lower_bound=lower_bound,
                                        upper_bound=upper_bound,
                                        algorithm=nlopt.LN_BOBYQA,
                                        maxeval=maxiter, verbose=verbose)
    ...
    print('Optimized log-likelihood:', ll_opt, ll_model)
    print('theta:', theta)
...
for namemodel in model_list:
    ...
    if namemodel == "SI":
        ...
        for optimizationstate in opt_list:
            ...
            lower_bound = [0.01, 0.01, 0, 0.01]
                                  ...
                                  verbose=verbose, maxiter=400, Tini=50, Tfin=0, learn_rate=0.005, 
                                  schedule= "cauchy")
        if done: print(("\n" + namemodel + " : done\n"))
    ...
    if namemodel == "SI2N":
        ...
        for optimizationstate in opt_list:
            ...
            # will try wacky parameter values. We in particular want to exclude values with
            # very long times, as they will take a long time to evaluate.
            upper_bound = [100, 100, 16, 1, 0.999 , 0.99]
    ...
    if namemodel == "IM":
        ...
        for optimizationstate in opt_list:
            ...
            if optimizationstate == "anneal_hot":
                params = (0.1, 0.1, 0.01, 0.01, 0.5, 0.8)
            elif optimizationstate == "anneal_cold":
                params = (popt[0], popt[1], popt[2], popt[3], popt[4], popt[5])
            else :
                params = (popt[0], popt[1], popt[2], popt[3], popt[4], popt[5])
            ...
            # will try wacky parameter values. We in particular want to exclude values with
            # very long times, as they will take a long time to evaluate.
            upper_bound = [120, 120, 5, 5, 16, 0.99]
    ...
    if namemodel == "IM2N":
        ...
        for optimizationstate in opt_list:
            ...
            if optimizationstate == "anneal_hot":        
                params = (0.1, 0.1, 0.8, 0.01, 0.01, 0.5, 0.1, 0.8)
            elif optimizationstate == "anneal_cold":
                params = (popt[0], popt[1], popt[2], popt[3], popt[4], popt[5], popt[6], popt[7])
            else :
                params = (popt[0], popt[1], popt[2], popt[3], popt[4], popt[5], popt[6], popt[7])
            ...
            # will try wacky parameter values. We in particular want to exclude values with
            # very long times, as they will take a long time to evaluate.
            upper_bound = [120, 120, 1, 5, 5, 16, 0.5, 0.99]
    ...
    if namemodel == "IM2m":
        ...
        for optimizationstate in opt_list:
            ...
            if optimizationstate == "anneal_hot":        
                params = (0.1, 0.1, 0.1, 0.1, 0.01, 0.01, 0.5, 0.5, 0.8)
            elif optimizationstate == "anneal_cold":
                params = (popt[0], popt[1], popt[2], popt[3], popt[4], popt[5], popt[6], popt[7], popt[8])
            else :
                params = (popt[0], popt[1], popt[2], popt[3], popt[4], popt[5], popt[6], popt[7], popt[8])
            ...
            # will try wacky parameter values. We in particular want to exclude values with
            # very long times, as they will take a long time to evaluate.
            upper_bound = [250, 250, 10, 10, 5, 5, 16, 0.95, 0.99]
