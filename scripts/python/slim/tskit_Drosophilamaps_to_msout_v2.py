...
parser.add_option("--start", help="Replicate to start on.",
...
def slimulation(repnumber):
    ...
    locationlist = []
    ratelist = []
    recombinationratefile = 'rfile_%s.txt' % str(repnumber)
    ...
    if options.paramsfile != 'None':
        ...
        os.system("%s -d rep=%r -d tmig=%r -d pmig=%r -d divtime=%r %s > %s_%r_sliminfo.txt" % (options.slimexecutable, repnumber, tmig, pmig, options.divtime, slimfilerep, options.prefix, repnumber))
        os.system('rm %s' % (slimfilerep))

    else: ### Run the SLiM model and load the resulting .trees
        slimfilerep = options.prefix+'_'+str(repnumber)+'.slim'
        os.system('cp ../%s %s' % (options.slimfile, slimfilerep))
        os.system("%s -d rep=%r -d divtime=%r %s > %s_%r_sliminfo.txt" % (options.slimexecutable, repnumber, options.divtime, slimfilerep, options.prefix, repnumber))
        os.system('rm %s' % (slimfilerep))
    ...
    if options.dfe in ('neutral', 'sweep', 'bgs'):
        if len(locationlist) > 2:
            ...
            ts = pyslim.recapitate(orig_ts, recombination_map=recmap, ancestral_Ne=1250)
        else:
            ts = pyslim.recapitate(orig_ts, recombination_rate=ratelist[0], ancestral_Ne=1250)
    ...
    if options.dfe in ('neutral', 'sweep'):
        ...
        ts = msprime.sim_mutations(ts, rate=mutrate, model=msprime.SLiMMutationModel(type=5), keep=True)
...
folder = options.prefix+'_'+str(options.divtime)
...
for i in range(options.start,options.numreps+1):
    slimulation(i)
