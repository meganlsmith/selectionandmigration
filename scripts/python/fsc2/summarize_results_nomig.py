...
for directory in all_directories:
    ...
    for replicate in range(100):
        ...
        filename = os.path.join(directory, 'rep_%s' % str(replicate), 'Nomig_%s' % str(time), 'Nomig_%s.bestlhoods' % str(time))
...
results_df.to_csv(path_or_buf='summary_results_nomig.csv')
